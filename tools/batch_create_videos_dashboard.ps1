...
function Get-DashboardLineColor($Line) {
    ...
    if ($text.StartsWith('ThreadGens LIVE BATCH MONITOR')) { return [ConsoleColor]::Cyan }
    if ($text.StartsWith('[') -and $text -match 'APPROVED\s+\d+/\d+') {
        if ($text -match 'rejected\s+[1-9]') { return [ConsoleColor]::Yellow }
        return [ConsoleColor]::Green
    }
    if ($text.StartsWith('Slots:')) { return [ConsoleColor]::DarkCyan }
    ...
    if ($text.StartsWith('GPU ')) { return [ConsoleColor]::Magenta }
    if ($text.StartsWith('Ollama gate:')) { return [ConsoleColor]::Yellow }
    if ($text.StartsWith('SLOT ATT')) { return [ConsoleColor]::Cyan }
    if ($text.StartsWith('--------')) { return [ConsoleColor]::DarkGray }
    if ($text -eq 'RECENT EVENTS') { return [ConsoleColor]::Cyan }
    if ($text.StartsWith('DEBUG LOG:')) { return [ConsoleColor]::DarkGray }

    if ($text -match '^\d\d\d\s+\d\d\d\d\s+(?<stage>[A-Z0-9 ]+)\s+') {
        $stage = $Matches.stage.Trim()
        switch ($stage) {
            'FAILED' { return [ConsoleColor]::Red }
            'APPROVED' { return [ConsoleColor]::Green }
            'OLLAMA' { return [ConsoleColor]::Yellow }
            'NOVELTY' { return [ConsoleColor]::Yellow }
            'SCRIPT READY' { return [ConsoleColor]::Cyan }
            'PROFILES' { return [ConsoleColor]::Cyan }
            'IMAGES' { return [ConsoleColor]::Cyan }
            'RENDER QUEUE' { return [ConsoleColor]::Cyan }
            'STARTING' { return [ConsoleColor]::Cyan }
            'TTS' { return [ConsoleColor]::DarkYellow }
            'VIDEO' { return [ConsoleColor]::Magenta }
            'VALIDATE' { return [ConsoleColor]::White }
            'FINALIZE' { return [ConsoleColor]::Green }
            'P2 AUDIT' { return [ConsoleColor]::Green }
            'SAVE' { return [ConsoleColor]::Green }
            default { return [ConsoleColor]::Gray }
        }
    }

    if ($text -match '^\s+\d\d:\d\d:\d\d\s+') {
...
function Render-Dashboard([switch]$Final) {
    ...
    [void]$lines.Add("ThreadGens LIVE BATCH MONITOR  |  $statusWord  |  runtime $(Format-Duration $elapsed)  |  Q/Esc=STOP")
